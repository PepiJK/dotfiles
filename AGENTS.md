...
| `.agents/skills/`   | Per-skill folders (`SKILL.md` + files) | `~/.gemini/antigravity-cli/skills/<skill>/`, `~/.agents/skills/<skill>/` (Linux) / `$UserHome\.gemini\antigravity-cli\skills\<skill>\`, `$UserHome\.agents\skills\<skill>\` (Windows junctions)                                                                                                              |
...
- **`.agents/skills/`** contains one folder per repository-managed skill (e.g. `pepi-unslop`, `pepi-setup-project-templates`, `pepi-preplanning`, `pepi-agent-writing`, listed explicitly in `setup.sh`/`setup.ps1`). Each folder contains `SKILL.md` and any supporting files, and is linked as a whole into `~/.gemini/antigravity-cli/skills/<skill>/` and `~/.agents/skills/<skill>/` (used by GitHub Copilot).
